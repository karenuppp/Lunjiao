...
def _resolve_permissions(user_id: str) -> tuple[str, list[int] | None, bool]:
    ...
    ('none', None, False) when user_id is 'default' or not found.
    """
    kb_scope = "none"
    db_scope = None
...
async def download_skill_file(download_id: str):
    """Download a file generated by a skill script (e.g. official-document-format .docx).

    Files are stored in the download_dir configured in settings.
    """
    from app.config import settings

    download_dir = getattr(settings, 'download_dir', str(_Path(__file__).parent.parent.parent / "downloads"))
    file_path = _Path(download_dir) / download_id

    if not file_path.exists():
        from fastapi import HTTPException
        raise HTTPException(status_code=404, detail="文件不存在或已过期")

    # Determine filename from stored metadata or default
    meta_path = _Path(download_dir) / f"{download_id}.meta"
